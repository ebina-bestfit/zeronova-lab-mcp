...
server.tool("check_security_headers", "Check HTTP security headers on a webpage. Inspects 6 key security headers: Strict-Transport-Security (HSTS), Content-Security-Policy (CSP), X-Content-Type-Options, X-Frame-Options, Referrer-Policy, and Permissions-Policy. Returns each header's presence, value, and pass/warn/fail status with specific recommendations, plus an overall security score (0-100).", urlSchema, async ({ url }) => {
    try {
        const validUrl = validateUrl(url);
...
server.tool("run_freelance_delivery_audit", "Run a pre-delivery quality audit for freelance web projects. Checks link integrity, page speed, alt attributes, color contrast, meta tags, OGP configuration, favicon, and security headers. Returns a score (0-100) with per-item results. Also lists manual-check items for proofreading and invoicing.", urlSchema, async ({ url }, extra) => {
    try {
        const validUrl = validateUrl(url);
