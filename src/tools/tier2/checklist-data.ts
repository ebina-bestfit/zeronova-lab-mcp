...
function getSiteConfigData(results: CollectedToolResults) {
  const r = results.siteConfig;
  if (!r || "error" in r) return null;
  return r.data;
}

function getSecurityHeadersData(results: CollectedToolResults) {
...
function evalNoBrokenLinks(
  ...
  const trulyBroken = links.links.filter(
    (l) => l.status >= 400 && !l.warning,
  );
  const botBlocked = links.links.filter(
    (l) => l.status >= 400 && !!l.warning,
  );
  ...
  if (trulyBroken.length > 0) {
    const urls = trulyBroken
      ...
      .join("、");
    parts.push(
      `${trulyBroken.length}件のリンク切れ: ${urls}${trulyBroken.length > 3 ? ` 他${trulyBroken.length - 3}件` : ""}`,
    );
  }
  ...
  if (botBlocked.length > 0) {
    const urls = botBlocked
      ...
      .join("、");
    parts.push(
      `${botBlocked.length}件はボットブロック（${urls}）— ブラウザで直接確認してください`,
    );
  }
...
function evalRobotsTxt(
  results: CollectedToolResults,
): { status: CheckStatus; detail?: string } {
  const siteConfig = getSiteConfigData(results);
  if (!siteConfig) return toolError("サイト設定チェッカー");
...
function evalSecurityHeaders(
  results: CollectedToolResults,
): { status: CheckStatus; detail?: string } {
  const sec = getSecurityHeadersData(results);
  if (!sec) return toolError("セキュリティヘッダーチェッカー");
