...
type RedirectStatusCode = (typeof VALID_STATUS_CODES)[number];
 ...
 */
function isValidStatusCode(code: number): code is RedirectStatusCode {
  return (VALID_STATUS_CODES as readonly number[]).includes(code);
}
 ...
 */
function escapeExtension(ext: string): { escaped: string; valid: boolean } {
 ...
 */
function statusToFlag(code: RedirectStatusCode): string {
  switch (code) {
...
export function handleGenerateHtaccess(input: HtaccessInput): HtaccessResult {
  const issues: string[] = [];
