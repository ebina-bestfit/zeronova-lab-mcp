...
type SchemaType = (typeof SUPPORTED_SCHEMA_TYPES)[number];
 ...
 */
function isSupportedType(type: string): type is SchemaType {
  return (SUPPORTED_SCHEMA_TYPES as readonly string[]).includes(type);
}
...
export function handleGenerateJsonLd(input: JsonLdInput): JsonLdResult {
  const issues: string[] = [];
  ...
  if (!hasValidType) {
    issues.push(
      `Unsupported schema type "${schemaType}". Supported types: ${SUPPORTED_SCHEMA_TYPES.join(", ")}. The JSON-LD will still be generated but may not follow Schema.org conventions.`,
    );
  }
  ...
  if (missingRequiredFields.length > 0) {
    issues.push(
      `Missing required fields for ${schemaType}: ${missingRequiredFields.join(", ")}`,
    );
  }

  // Build JSON-LD object — using JSON.stringify for safe serialization (checklist 2-D)
  const sanitizedData = sanitizeData(input.data) as Record<string, unknown>;
