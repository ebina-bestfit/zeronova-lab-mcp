 ...
 */
function analyzeTitleLength(len: number): "good" | "short" | "long" {
 ...
 */
function analyzeDescriptionLength(len: number): "good" | "short" | "long" {
  if (len < 70) return "short";
  if (len > 160) return "long";
  return "good";
}

export function handleGenerateMetaTags(input: MetaTagInput): MetaTagResult {
  const issues: string[] = [];
  const tags: string[] = [];
  let tagCount = 0;
